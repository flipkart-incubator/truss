...
import urlHelper from "../lib/helpers/url-utils";
...
function SimpleRouter() {
    ...
    if (!urlHelper.getHash(url)) {
        urlHelper.setUrl(urlHelper.setHash('login', url));
    }
}
...
SimpleRouter.prototype = (function() {
	return {
		...
		extractParameters: function(route, fragment) {
			...
			_.each(paramValues, function(param, index) {
		...
		loadUrl: function() {
            ...
            if(self.preLoad){
                self.preLoad(fragment).then(() => {
                    hasMatched = _.any(self.routes, function(route) {
		...
		listen: function() {
			...
			if(this.mode === "history") {
				$(window).on('popstate', $.proxy(self.checkUrl, self));
			} else {
				$(window).on('hashchange', $.proxy(self.checkUrl, self));
			}
		...
		stop: function() {
			...
			$(window).off('popstate', self.checkUrl).off('hashchange', self.checkUrl);
		},
